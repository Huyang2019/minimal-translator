...
content.js──sendMessage──▶  background.js (bgTranslateStream / bgTranslate / bgTts / shouldAutoTranslate)
background.js─tabs.sendMessage─▶ content.js (streamSegment / streamDone)
```
...
1. `translatePage()` 先将节点拆分为**视口内**和**屏外**两组
2. 视口内节点优先翻译（await），完成后立即标记 `isTranslated` 并启动 live 模式
3. 屏外节点在后台并行翻译（不阻塞返回）
4. 每个批次走 `bgTranslateStream`：background.js 开启 `stream: true`，解析 SSE，遇到 `<<<###>>>` 分隔符立即通过 `tabs.sendMessage` 推送 `streamSegment` 给 content.js
5. content.js 的 `streamRegistry` Map 接收段落，调用 `applyTranslation()` 即时渲染，不等整批完成
...
- **translateNodes()**：去重 → LRU 缓存命中直接应用 → 短文本（≤30字符，50条/批）与长文本（10条/批）分批 → 5路并发流式
...
- **MutationObserver 监听 attributes**（class/style/hidden/aria-hidden）：弹窗/轮播切换时 CSS class 变化也能触发翻译

### 自动翻译启动逻辑（content.js autoCheck IIFE）

1. 从 `chrome.storage.local` 恢复 `hoverEnabled`（页面刷新后不丢失）
...
- 选中文字后 `mouseup` 触发，400ms 防抖后调用 `showTooltip`
- 悬浮框内点击不触发 `mouseup` 重定位（`contains` 判断）
- 朗读引擎：`browser`（智能选最优英语声音）或 `mimo`（小米 MiMo TTS，background.js 中转）
- `speakText()` 根据 `chrome.storage.local` 里的 `ttsEngine` 动态选择引擎
